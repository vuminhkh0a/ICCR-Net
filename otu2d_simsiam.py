...
nohup python3 [OTU2D]-simsiam.py > log2.txt 2>&1 &
tail -f log2.txt
'''
...
# SimCLR / SimSiam paper augmentation:
#   RandomResizedCrop -> HFlip -> ColorJitter(p=0.8) -> Grayscale(p=0.2)
#   -> GaussianBlur(p=0.5).
import torchvision.transforms as T

class TwoViewSimCLR:
    def __init__(self, H=256, W=256, s=1.0, blur_p=0.5):
        self.t = T.Compose([
            T.RandomResizedCrop((H, W), scale=(0.2, 1.0)),
    ...
    def __call__(self, img_pil):
        return self.t(img_pil), self.t(img_pil)

class SimSiam(nn.Module):
    """Encoder f -> 3-layer projector g -> 2-layer predictor h.

    Loss: 0.5 * (D(p1, sg(z2)) + D(p2, sg(z1))) where D = -cos.
    """
    def __init__(self, proj_hidden=2048, proj_out=2048, pred_hidden=512,
                 pretrained=True):
        super().__init__()
        self.backbone = VGG16Encoder(pretrained=pretrained)
        self.gap = nn.AdaptiveAvgPool2d(1)
        # 3-layer projector with BN after every Linear (incl. last).
        self.projector = nn.Sequential(
            nn.Linear(512, proj_hidden, bias=False),
            nn.BatchNorm1d(proj_hidden), nn.ReLU(inplace=True),
        ...
        self.predictor = nn.Sequential(
            nn.Linear(proj_out, pred_hidden, bias=False),
...
def train_simsiam(model, loader, *, epochs, base_lr, device=device,
                  momentum=0.9, weight_decay=1e-4, save_path=None):
    model = model.to(device)
    steps_per_epoch = max(1, len(loader))
    total_steps = epochs * steps_per_epoch
    opt = torch.optim.SGD(model.parameters(), lr=base_lr,
                          momentum=momentum, weight_decay=weight_decay)
    # Paper: cosine decay, no warmup. Predictor LR is fixed in the original paper;
    # here we keep one schedule (the simpler choice -- noted as a deviation).
    step = 0
    history = []
    for ep in range(epochs):
        model.train()
        ...
        history.append(avg)
        print(f'Ep {ep+1:3d}/{epochs} | simsiam loss {avg:.4f} | lr {lr:.2e} | {time.time()-t0:.1f}s')
    if save_path:
...
ssl_ds = SSLArrayDataset(X_train_data, TwoViewSimCLR(H=IMG_HEIGHT, W=IMG_WIDTH))
...
simsiam = SimSiam(proj_hidden=2048, proj_out=2048, pred_hidden=512)

TRAIN_FLG = 0
SSL_CKPT = 'weight/VGG16_SimSiam_encoder.pt'

if TRAIN_FLG:
    print('===== Training SimSiam =====')
    history = train_simsiam(
        simsiam, ssl_loader,
        epochs=EPOCHS, base_lr=base_lr,
        save_path=SSL_CKPT,
    )
else:
    print('===== Loading SimSiam encoder weights =====')
    simsiam.backbone.load_state_dict(torch.load(SSL_CKPT, map_location=device))
...
backbone_ckpt = 'weight/VGG16_SimSiam_encoder.pt'
...
if TRAIN_FLG:
    ...
    fit_seg(seg_model, train_loader, valid_loader,
            ...
            patience_es=30, patience_rl=10, rl_factor=0.1, min_lr=1e-8,
            best_path='weight/SimSiam_Best.pt')


seg_model.load_state_dict(torch.load('weight/SimSiam_Best.pt', map_location=device))
...
print(f'BEST TEST SimSiam | Dice: {dice:.4f} | IoU: {iou:.4f} | Precision: {prec:.4f} | Recall: {rec:.4f} | HD95: {hd95:.4f}')
