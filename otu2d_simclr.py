...
class SimCLR(nn.Module):
    """Encoder f -> 2-layer projector g; trained with NT-Xent."""
    ...
    def forward(self, x1, x2):
        ...
        return self.nt_xent(z1, z2, self.T)
    ...
    def nt_xent(z1, z2, T):
        ...
        pos = torch.cat([(z1 * z2).sum(dim=1),       # (2B,)
                         (z2 * z1).sum(dim=1)], dim=0) / T
        neg_logsumexp = torch.logsumexp(sim, dim=1)
        return (-pos + neg_logsumexp).mean()

def train_simclr(model, loader, *, epochs, base_lr, warmup_epochs, device=device,
    ...
    for ep in range(epochs):
        ...
        history.append(avg)
        print(f'Ep {ep+1:3d}/{epochs} | simclr loss {avg:.4f} | lr {lr:.2e} | {time.time()-t0:.1f}s')
    if save_path:
...
simclr = SimCLR(proj_hidden=2048, proj_out=128, temperature=0.5)

TRAIN_FLG = 0
SSL_CKPT = 'weight/VGG16_SimCLR_encoder.pt'
...
if TRAIN_FLG:
    ...
    print('===== Loading SimCLR encoder weights =====')
    simclr.backbone.load_state_dict(torch.load(SSL_CKPT, map_location=device))
...
backbone_ckpt = 'weight/VGG16_SimCLR_encoder.pt'
...
if TRAIN_FLG:
    ...
    fit_seg(seg_model, train_loader, valid_loader,
            ...
            patience_es=30, patience_rl=10, rl_factor=0.1, min_lr=1e-8,
            best_path='weight/SimCLR_Best.pt')

seg_model.load_state_dict(torch.load('weight/SimCLR_Best.pt', map_location=device))
...
print(f'BEST TEST SimCLR | Dice: {dice:.4f} | IoU: {iou:.4f} | Precision: {prec:.4f} | Recall: {rec:.4f} | HD95: {hd95:.4f}')