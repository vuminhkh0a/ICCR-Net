...
class MoCoEncoder(nn.Module):
    """VGG16 -> GAP -> MoCo-v2 MLP projector (2-layer with BN+ReLU+BN)."""
    ...
    def forward(self, x):
        b, _ = self.backbone(x)
        h = self.gap(b).flatten(1)
        return self.proj(h)


class MoCo(nn.Module):
    """MoCo v2: query/key encoders + dictionary queue + InfoNCE."""
    def __init__(self, proj_dim=256, queue_size=4096, momentum=0.999, temperature=0.07):
        ...
        self.m = momentum; self.T = temperature
        self.K = queue_size
        q = torch.randn(proj_dim, queue_size)
...
def train_moco(model, loader, *, epochs, base_lr, warmup_epochs, device=device,
               save_path=None):
    model = model.to(device)
    ...
    for ep in range(epochs):
        ...
        for v1, v2 in loader:
            ...
            lr = warmup_cosine_lr(step, base_lr, warmup_steps, total_steps)
        ...
        history.append(avg)
        print(f'Ep {ep+1:3d}/{epochs} | moco loss {avg:.4f} | lr {lr:.2e} | {time.time()-t0:.1f}s')
    if save_path:
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        torch.save(model.encoder_q.backbone.state_dict(), save_path)
        print(f'Saved encoder_q backbone to {save_path}')
    return history
...
base_lr       = 1e-4
moco = MoCo(proj_dim=256, queue_size=4096, momentum=0.999, temperature=0.07)

TRAIN_FLG = 0
SSL_CKPT = 'weight/VGG16_MoCo_encoder.pt'

if TRAIN_FLG:
    print('===== Training MoCo v2 =====')
    history = train_moco(
        moco, ssl_loader,
    ...
    print('===== Loading MoCo encoder weights =====')
    moco.encoder_q.backbone.load_state_dict(torch.load(SSL_CKPT, map_location=device))
...
backbone_ckpt = 'weight/VGG16_MoCo_encoder.pt'
...
if TRAIN_FLG:
    ...
    fit_seg(seg_model, train_loader, valid_loader,
            ...
            patience_es=30, patience_rl=10, rl_factor=0.1, min_lr=1e-8,
            best_path='weight/MoCo_Best.pt')

seg_model.load_state_dict(torch.load('weight/MoCo_Best.pt', map_location=device))
...
print(f'BEST TEST Moco | Dice: {dice:.4f} | IoU: {iou:.4f} | Precision: {prec:.4f} | Recall: {rec:.4f} | HD95: {hd95:.4f}')